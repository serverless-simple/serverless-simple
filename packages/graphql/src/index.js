...
export default function (opts={}) {
  return async function (ctx, next) {
    ...
    // pass-through if the path doesn't match
    if (!path.startsWith(opts.path)) {
      await next()
      return
    }
    
    // see if this is the playground
    if (path.startsWith(`${opts.path}/playground`)) {
      res.body = playgroundHtml
      return
    }
    
    // instantiate the server handler
    const handler = opts.init(ctx)
    
    // respond
    ctx.respondWith(await handler(req.raw))
    ctx.responded
  }
}
  ...
  <body>
    ...
    <script type="text/javascript">
      window.addEventListener('load', function (event) {
        ...
        GraphQLPlayground.init(root, { endpoint: window.location.href.replace(/\\/playground$/, '') })
      })
    </script>
  </body>
</html>
